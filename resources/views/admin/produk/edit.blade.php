    ...
    <div class="space-y-6">
        ...
        <div class="page-header">
            <div>
                ...
                <p class="page-subtitle">Perbarui informasi produk: <span class="font-bold text-blue-600">{{ $produk->nama_produk }}</span></p>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-card">
            ...
            <form action="{{ route('admin.produk.update', $produk->id_produk) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="id_umkm" required
                            ...
                            @foreach($umkms as $u)
                                <option value="{{ $u->id_umkm }}"
                                        {{ old('id_umkm', $produk->id_umkm) == $u->id_umkm ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="harga"
                               value="{{ old('harga', $produk->harga) }}" step="1000" min="0" required
                               class="form-input pl-10 @error('harga') border-red-400 @enderror">
                    </div>
                </div>
                ...
                <div>
                    <label class="form-label">Foto Produk <span class="text-slate-400 font-normal normal-case">(kosongkan jika tidak diubah)</span></label>

                    @if($produk->foto)
                        <div class="mb-3 p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-3" x-show="!photoPreview">
                    ...
                    <div class="upload-zone"
                         ...
                         ">
                        <input type="file" name="foto" ref="fotoInput" class="hidden" accept="image/*"
                               @change="
                        ...
                        <template x-if="photoPreview">
                            <div class="flex flex-col items-center">
                                <img :src="photoPreview" class="w-32 h-32 rounded-xl object-cover mx-auto border-2 border-amber-300 shadow-md mb-2">
                                <p class="text-xs text-amber-600 font-semibold">✓ Foto baru dipilih — klik untuk ganti</p>
                            </div>
                        </template>
