<x-app-layout>
    <div class="space-y-6" x-data="{ deleteModal: false, deleteUrl: '' }">
            ...
            <form action="{{ route('admin.umkm.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama UMKM, pemilik, atau kategori..." class="form-input pl-10 py-2.5">
                </div>
                        ...
                        @forelse($umkms as $item)
                            <tr>
                                <td class="pl-6">
                                    ...
                                    @else
                                        <div
                                            class="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-100 to-teal-100 flex items-center justify-center text-emerald-700 font-black text-sm">
                                ...
                                <td>
                                    @if($item->hasValidLocationLink())
                                        <a href="{{ $item->link_lokasi }}" target="_blank" rel="noopener noreferrer"
        ...
        ============================================= --}}
        <div x-show="deleteModal" x-cloak class="modal-overlay" x-transition:enter="transition ease-out duration-200"
            ...
            <div class="modal-box" @click.stop>
                ...
                <p class="text-sm text-slate-600 mb-6 p-4 bg-red-50 rounded-xl border border-red-100">
                    ⚠️ Anda yakin ingin menghapus data UMKM ini beserta semua produk yang terkait? Data yang sudah
