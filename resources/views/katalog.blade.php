    ...
    <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200/80">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8 text-sm font-semibold">
                <a href="{{ route('home') }}"
                    class="text-slate-600 hover:text-emerald-600 transition-colors">Beranda</a>
                <a href="{{ route('home') }}#profil-desa"
                    class="text-slate-600 hover:text-emerald-600 transition-colors">Profil Desa</a>
                <a href="{{ route('katalog') }}" class="text-emerald-600">Katalog Produk</a>
                <a href="{{ route('home') }}#umkm" class="text-slate-600 hover:text-emerald-600 transition-colors">Mitra
                    UMKM</a>
                <a href="{{ route('home') }}#berita"
                    class="text-slate-600 hover:text-emerald-600 transition-colors">Kabar Desa</a>
            </nav>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-emerald-100/80 text-sm mt-1 max-w-xl">
                        Koleksi lengkap hasil kerajinan anyaman bambu, olahan pangan, dan produk unggulan warga Desa
                        Selotinatah.
                    </p>
                </div>
            </div>
            ...
            <form action="{{ route('katalog') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-3">
                <div class="md:col-span-6 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama produk atau deskripsi..." class="form-input pl-10">
                </div>
                ...
                <div class="md:col-span-2 flex gap-2">
                    ...
                    @if(request()->filled('search') || request()->filled('kategori'))
                        <a href="{{ route('katalog') }}" class="btn-secondary px-3 text-xs justify-center"
                            title="Reset filter">
                            Reset
                        </a>
                    @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            @empty
                <div class="col-span-full bg-white p-12 rounded-3xl border border-dashed border-slate-300 text-center">
                    <div
                        class="w-14 h-14 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto mb-3">
                    ...
                    <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
                        Maaf, tidak ada produk yang cocok dengan pencarian atau filter yang Anda pilih. Coba gunakan kata
                        kunci lain.
                    </p>
                    <div class="mt-4">
